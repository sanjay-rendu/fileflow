...
class FileStorageDriver(StorageDriver):
    ...
    def get_filename(self, dag_id, task_id, data_id, execution_date):
        return os.path.join(
            self.prefix,
            dag_id,
            task_id,
            "{}_{}".format(str(data_id),str(self.execution_date_string(execution_date)))
        )

    def get_path(self, dag_id, task_id):
        return os.path.join(self.prefix, dag_id, task_id)

    def read(self, dag_id, task_id, data_id, execution_date, encoding='utf-8'):
        filename = self.get_filename(dag_id, task_id, data_id, execution_date)
    ...
    def get_read_stream(self, dag_id, task_id, data_id, execution_date):
        filename = self.get_filename(dag_id, task_id, data_id, execution_date)

        f = codecs.open(filename, 'rb')
        return f

    def write(self, dag_id, task_id,  data_id, execution_date, data, *args, **kwargs):
